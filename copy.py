...
def get_mine(): # to go to basic mine from the map
    ...
    find_another()
    while True:
        if all(abs(a - t) < 25 for a, t in zip(check_color(point_search_back), (50, 48, 40, 255))):# if mine found
            print("mine found")
...
def get_mine(): # to go to basic mine from the map
    ...
    print("True" if all(abs(a - t) < 25 for a, t in zip(img.getpixel((point_checking[0], point_checking[1])), (34, 108, 137, 255))) else "False", img.getpixel((point_checking[0], point_checking[1])))